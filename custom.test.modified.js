...
describe('Custom Tests', function () {
  ...
  it('[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances', async () => {
    ...
    // let res = await token.balanceOf(omniBridge.address);
    // console.log('1 omni', res.toString());
    ...
    expect(aliceDepositAmount).to.be.equal(omnibridgeBalance);

    // res = await token.balanceOf(tornadoPool.address);
    // console.log('1 tornado', res.toString());
    // res = await token.balanceOf(omniBridge.address);
    // console.log('2 omni', res.toString());
    ...
    expect(aliceDepositAmount).to.be.equal(tornadoBalance);
    
    // res = await token.balanceOf(tornadoPool.address);
    // console.log('2 tornado', res.toString());
    // res = await token.balanceOf(omniBridge.address);
    // console.log('3 omni', res.toString());

    // Alice withdraws a part of his funds from the shielded pool
    const aliceWithdrawAmount = utils.parseEther('0.08')
    const recipientAddress = '0xDeaD00000000000000000000000000000000BEEf'
    const aliceChangeUtxo = new Utxo({ amount: aliceDepositAmount.sub(aliceWithdrawAmount), keypair: aliceDepositUtxo.keypair })
    await transaction({
      tornadoPool,
    ...
    const aliceBalance = await token.balanceOf(recipientAddress)
  ...
  it('[assignment] iii. see assignment doc for details', async () => {
    ...
    // Alice withdraws all of his funds from the shielded pool
    const aliceWithdrawAmount = utils.parseEther('0.07')
    const recipient = '0xfeee00000000000000000000000000000000ffff'
